...
4) Make a standard class (Camera) and an Aspect class (Logger) that will advise the standard class and add beans for each.
...
Note: this can also be done with annotations, see Camera and Logger classes
...
#################################################################
In order to execute an aspect, Spring takes the original advised class (Camera) and makes a subclass that also extends classes that add methods that allow it to be advised.
There was a bug that used to exist for classes that implemented interfaces, that is best displayed in an example.
interface PhotoSnapper
class Camera implements PhotoSnapper
...
We would have an issue thrown due to the fact that it would take th interface PhotoSnapper and not Camera and add all the aop methods to the superclass of the interface. Therefore
System.out.println(camera instanceof Camera) => false 
...
target(us.brianolsen.spring.aop.Camera)
target(us.brianolsen.spring.aop.PhotoSnapper)
